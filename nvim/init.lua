...
require("lazy").setup({
	...
	},
	{
		...
		lazy = false,
		keys = {
			{ "-", function() require("oil").open(vim.fn.expand("%:p:h")) end, desc = "Open parent directory" },
			{ "_", function() require("oil").open(vim.fn.getcwd()) end, desc = "Open working directory" },
		},
		opts = {
	...
	},
	{
		...
		event = "VeryLazy",
		keys = {
			{ "<leader>h", function() require("spelunk").goto_bookmark_at_index(1) end, desc = "Go to Bookmark 1" },
			{ "<leader>j", function() require("spelunk").goto_bookmark_at_index(2) end, desc = "Go to Bookmark 2" },
			{ "<leader>k", function() require("spelunk").goto_bookmark_at_index(3) end, desc = "Go to Bookmark 3" },
			{ "<leader>l", function() require("spelunk").goto_bookmark_at_index(4) end, desc = "Go to Bookmark 4" },
		},
		opts = {
			enable_persist = true,
			...
			orientation = {
				bookmark_dimensions = function()
					return {
						base = { width = vim.o.columns - 4, height = vim.o.lines - 4 },
						line = 2,
						col = 2,
					}
				end,
	...
	},
	{
		...
		lazy = false,
		config = function()
			-- Crucial: do not save blank utility buffers or terminal buffers into the session file
			vim.opt.sessionoptions = { "buffers", "curdir", "folds", "globals", "help", "tabpages", "winsize" }

			require("mini.sessions").setup({
				autoread = false,
				autowrite = false, -- We will handle this manually to ensure safe state
				directory = vim.fn.stdpath("data") .. "/sessions",
			})

			-- Safe Session Saving: strip out dynamic UI elements before photographing the layout
			vim.api.nvim_create_autocmd("VimLeavePre", {
				callback = function()
					-- 1. Disable NoNeckPain so its padding buffers don't get saved into the session
					if vim.g.no_neck_pain_enabled then
						vim.cmd("NoNeckPain")
					end
					
					-- 2. Wipe out any hidden / unlisted buffers (like Oil virtual paths) that break restore
					for _, bufnr in ipairs(vim.api.nvim_list_bufs()) do
						if vim.bo[bufnr].buftype == "nofile" or vim.bo[bufnr].filetype == "oil" then
							pcall(vim.api.nvim_buf_delete, bufnr, { force = true })
						end
					end

					-- 3. Now it is safe to photograph the session
					local session_name = vim.fn.getcwd():gsub("/", "%%")
					require("mini.sessions").write(session_name)
				end,
			})

			-- Safe Session Loading: restore the bare files first, THEN apply the dynamic UI
			vim.api.nvim_create_autocmd("VimEnter", {
				callback = function()
					if vim.fn.argc(-1) == 0 then
						local session_name = vim.fn.getcwd():gsub("/", "%%")
						
						-- Read the session containing only real code files
						pcall(require("mini.sessions").read, session_name)
						
						-- Wait for Neovim to settle, then dynamically wrap the restored files in NoNeckPain
	...
	},
	{
		...
		opts = {
			width = 80,
			autocmds = {
				enableOnVimEnter = false, -- Handled dynamically by our session restore hook
			},
			buffers = {
				right = { enabled = true },
				left = { enabled = true },
